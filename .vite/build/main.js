...
var require_connection = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	var { parse, serialize } = require_dist$2();
	var stream = require_stream();
	var { getStream } = stream;
	var flushBuffer = serialize.flush();
	var syncBuffer = serialize.sync();
	var endBuffer = serialize.end();
	var Connection = class extends EventEmitter$4 {
		constructor(config) {
			super();
			config = config || {};
...
var require_helper = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	module.exports.getFileName = function(rawEnv) {
		...
		return env.PGPASSFILE || (isWin ? path$3.join(env.APPDATA || "./", "postgresql", "pgpass.conf") : path$3.join(env.HOME || "./", ".pgpass"));
	};
...
var require_lib = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	var TypeOverrides = require_type_overrides();
	var { DatabaseError } = require_dist$2();
...
var require_db = /* @__PURE__ */ __commonJSMin(((exports, module) => {
  ...
  `;
		const values = [
			...
			parseFloat(t.chargesAmount) || 0
		];
		return (await pool.query(insertQuery, values)).rows[0];
	}
  ...
  `, [vehicleNo])).rows;
	...
	*/
	var getReportData = async (filters = {}) => {
  ...
  `;
		...
		if (isLatestOnly) sql += ` LIMIT 1`;
		try {
			return (await pool.query(sql, queryParams)).rows;
		} catch (err) {
	...
	module.exports = {
		...
		printerDbMethods: {
			getAllFormats: async () => {
				...
				try {
					return (await client.query("SELECT format_id, format_name, is_default FROM printer_settings ORDER BY updated_at DESC")).rows;
				} catch (error) {
			...
			saveConfig: async (formatId, formatName, config) => {
				...
				try {
					...
					if (isDefault) await client.query("UPDATE printer_settings SET is_default = FALSE");
					let currentFormatId = formatId;
					if (currentFormatId) await client.query(`
        ...
        `, [
					...
					else currentFormatId = (await client.query(`
      ...
      `;
					if (Array.isArray(config.fields)) for (const f of config.fields) await client.query(fieldInsertQuery, [
						currentFormatId,
						f.fieldKey,
						f.display,
						f.x,
			...
			getConfig: async (formatId) => {
				...
				try {
					const masterRes = await pool.query("SELECT format_name, printer_type, paper_size, copies, bg_image, bg_opacity, is_default FROM printer_settings WHERE format_id = $1", [formatId]);
					if (masterRes.rows.length === 0) return null;
					const row = masterRes.rows[0];
					const fieldsRes = await pool.query(`SELECT 
...
var require_dist = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	var parser_delimiter_1 = require_dist$1();
	/**
...
//#endregion
//#region src/WhatsappService.js
var require_WhatsappService = /* @__PURE__ */ __commonJSMin(((exports, module) => {
...
var { Pool } = require_lib();
var { printerDbMethods } = require_db();
var { SerialPort } = require("serialport");
var { ReadlineParser } = require_dist();
var { initWhatsApp, sendWhatsAppMessage } = require_WhatsappService();
var port;
var pool = new Pool({
	user: "postgres",
	host: "localhost",
...
function createWindow() {
	...
	if (forgeDevUrl) mainWindow.loadURL(forgeDevUrl);
...
ipcMain.handle("save-ticket", async (event, payload) => {
    ...
    `;
		...
		return {
			success: true,
			ticketNo: (await client.query(query, values)).rows[0].ticket_number
		};
	} catch (error) {
...
ipcMain.handle("add-party", async (event, partyName) => {
    ...
    `;
		...
		return {
			success: true,
			partyId: (await client.query(query, values)).rows[0].id
		};
	} catch (error) {
		console.error("PG Ticket Save Insert error:", error);
		throw error;
...
ipcMain.handle("print-raw-ticket", async (event, payload) => {
	...
	try {
		let layout = null;
		if (formatId) layout = await printerDbMethods.getConfig(formatId);
		if (!layout) {
			const allFormats = await printerDbMethods.getAllFormats();
			const defaultFormat = allFormats?.find((f) => f.is_default === 1 || f.is_default === true) || allFormats?.[0];
			if (defaultFormat) layout = await printerDbMethods.getConfig(defaultFormat.format_id);
		}
		if (!layout) throw new Error("No valid printer format configuration found in database.");
...
$pd.add_PrintPage({
    ...
    ${(layout.fields || []).map((f) => {
			...
			const fieldValue = rawValue !== void 0 && rawValue !== null ? String(rawValue) : "";
			...
			let finalX = f.x || 0;
			const yPos = f.y || 0;
			if (finalX >= COL3_THRESHOLD) finalX += COL3_SHIFT;
			else if (finalX >= COL2_THRESHOLD) finalX += COL2_SHIFT;
			else finalX += COL1_SHIFT;
			return `
...
$pd.Print();
`;
		const psScriptPath = path.join(os.default.tmpdir(), `print_fast_${Date.now()}.ps1`);
		fs.default.writeFileSync(psScriptPath, psScript, "utf8");
		return new Promise((resolve, reject) => {
			(0, child_process.exec)(`powershell -NoProfile -ExecutionPolicy Bypass -File "${psScriptPath}"`, (error, stdout, stderr) => {
				if (fs.default.existsSync(psScriptPath)) fs.default.unlinkSync(psScriptPath);
				if (error) {
					console.error("[GDI PRINT ERROR]:", stderr || error.message);
					reject(/* @__PURE__ */ new Error(`Print failed: ${stderr || error.message}`));
				} else {
...
ipcMain.handle("send-sms", async (event, payload) => {
	...
	if (!phoneNumber || !message) throw new Error("Phone number and message content are required.");
...
async function checkIsLoopbackMode() {
	...
	try {
      ...
      LIMIT 1;
    `);
		if (res.rows.length > 0) return res.rows[0].config_value?.toLowerCase() === "loopback";
		return false;
	} catch (err) {
	...
	try {
		...
		port.pipe(new ReadlineParser({ delimiter: "\r\n" })).on("data", (data) => {
			...
			if (numericMatch) {
				...
				if (mainWindow && !mainWindow.isDestroyed()) mainWindow.webContents.send("serial-weight-data", liveWeight);
			}
		});
		port.on("error", (err) => {
...
app.whenReady().then(async () => {
	createWindow();
	initWhatsApp();
	if (await checkIsLoopbackMode()) startLoopbackMode();
	else initSerialPort();
});
//#endregion
